$(document).ready(function () {
    ...
    $('#selectCurr').click(function () {
        ...
        $.ajax({
            url: 'http://api.coinlayer.com/api/live?access_key=2c0b3b1184d5bf7caa13f5204cfec840&target=' + curr.value + '&symbols=YEE,YOC,YOYOW',
            type: 'GET',
            success: function (data) {
                if (data.success == true) {
                    console.log("success");
                    console.log(data);
                    $('#YEE-rate').text(data.rates.YEE);
                    $('#YOC-rate').text(data.rates.YOC);
                    $('#YOYOW-rate').text(data.rates.YOYOW);
                    $('#tableDiv').show();
